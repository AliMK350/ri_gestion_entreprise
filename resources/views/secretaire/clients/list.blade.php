...
<div class="content-wrapper">
    ...
    <section class="content"><div class="container-fluid">@include('_message')
        ...
        @forelse($getRecord as $v)<tr><td>{{ $v->name }}</td><td>{{ $v->company_name }}</td><td>{{ $v->email }}</td><td>{{ $v->phone }}</td><td>{{ $v->status == 0 ? 'Actif' : 'Inactif' }}</td><td><a href="{{ url('secretaire/clients/show/'.$v->id) }}" class="btn btn-info btn-sm">Historique</a> <a href="{{ url('secretaire/clients/edit/'.$v->id) }}" class="btn btn-primary btn-sm">Éditer</a> <a href="{{ url('secretaire/clients/delete/'.$v->id) }}" class="btn btn-danger btn-sm" onclick="return confirm('Supprimer ?')">Supprimer</a></td></tr>@empty<tr><td colspan="6" class="text-center">Aucun client.</td></tr>@endforelse
        </tbody></table></div></div>
    </div></section>
</div>
@endsection
