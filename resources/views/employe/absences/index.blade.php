...
<div class="content-wrapper">
    ...
    <section class="content"><div class="container-fluid">@include('_message')
        ...
        <div class="card"><div class="card-header"><h3>Absences</h3></div><div class="card-body p-0"><table class="table"><thead><tr><th>Date</th><th>Type</th><th>Motif</th><th>Justificatif</th></tr></thead><tbody>@forelse($absences as $a)<tr><td>{{ $a->date->format('d/m/Y') }}</td><td>{{ $a->half_day ? ($a->half_day == 'morning' ? 'Demi-journée matin' : 'Demi-journée après-midi') : 'Journée complète' }}</td><td>{{ $a->reason }}</td><td>@if(!empty($a->justification_file))<a href="{{ asset('storage/'.$a->justification_file) }}" target="_blank" class="btn btn-sm btn-outline-primary">Voir</a>@else<span class="text-muted">Aucun</span>@endif</td></tr>@empty<tr><td colspan="4">Aucune absence.</td></tr>@endforelse</tbody></table></div></div>
